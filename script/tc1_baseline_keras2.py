...
def load_data(train_path, test_path, gParameters):
    ...
    print('Loading data...')
    df_train = (pd.read_csv(train_path,header=None).values).astype('int8')
    df_test = (pd.read_csv(test_path,header=None).values).astype('int8')
    print('done')
    ...
    df_y_train = df_train[:,0].astype('int')-1
    print(df_y_train)
    df_y_test = df_test[:,0].astype('int')-1
    print(df_y_test)
    ...
    df_x_train = df_train[:, 1:seqlen].astype(np.int8)
    df_x_test = df_test[:, 1:seqlen].astype(np.int8)
...
def run(gParameters):
    ...
    train_file = data_utils.get_file(file_train, file_train, cache_subdir='Pilot1')
    test_file = data_utils.get_file(file_test, file_test, cache_subdir='Pilot1')
